...
interface DocumentReviewContentProps {
  ...
  onClose: () => void;
  /** Whether the preview image should be taller (for the side panel) */
...
export function DocumentReviewContent({
  ...
  const { data: suggestion } = useQuery({
    ...
    enabled: !!document && document.classificationStatus !== "revisto" && document.classificationStatus !== "classificado" && document.classificationStatus !== "arquivado",
    staleTime: 30_000,
  });
    ...
    <>
      ...
      <div className="sticky top-0 z-10 border-b bg-card px-5 py-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={onClose} className="shrink-0 rounded-md p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
            ...
            <div className="relative">
              <img
                ...
                className={cn(
                  ...
                  largePreview ? "max-h-[50vh]" : "max-h-[300px]"
                )}
