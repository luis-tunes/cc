...
function AccordionItem({
  ...
  return (
    <div className={`rounded-2xl border-2 transition-all duration-300 ${isOpen ? "border-primary/25 bg-primary/[0.03] shadow-md" : "border-transparent bg-transparent hover:bg-muted/30"}`}>
      <button
        onClick={onToggle}
        className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left sm:px-6 sm:py-5"
        aria-expanded={isOpen}
      >
        <span className={`text-sm font-bold transition-colors ${isOpen ? "text-foreground" : "text-foreground/80"}`}>{faq.q}</span>
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl transition-all duration-300 ${isOpen ? "bg-primary/10 rotate-180" : "bg-muted"}`}>
...
export function Pricing() {
  ...
  return (
    <section id="precos" className="relative py-20 sm:py-24 md:py-32 overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-b from-background via-emerald-50/20 dark:via-emerald-950/5 to-background" />
            ...
            <div className="lg:sticky lg:top-24">
              <div className="relative overflow-hidden rounded-3xl border-2 border-primary/30 bg-card p-7 shadow-2xl sm:p-9">
                ...
                <div className="relative">
                  ...
                  <div className="mt-7 h-0.5 bg-gradient-to-r from-transparent via-primary/30 to-transparent" />

                  <ul className="mt-7 space-y-3">
                    {FEATURES.map((f) => (
                      <li key={f} className="flex items-start gap-3 text-sm">
                        <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-200/50">
              ...
              <FadeIn delay={200} direction="none">
                <p className="mt-5 text-center text-sm text-muted-foreground">
                  ...
                  <a href="mailto:info@xtim.ai" className="font-bold text-primary hover:underline">
                    Fale connosco
                  </a>
                </p>
              </FadeIn>
